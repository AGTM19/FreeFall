...
text_out = Paragraph(text="The Maximum Force which acts on the Fallschirm is %d N, at time = %d s" % (
    max_force["max"][0], max_force["max"][1]))
...
area_slid = Slider(title="Set Maximum Area Fallschirm [m^2]", value=max_area, start=1, end=30, step=.1)
...
vel_slid = Slider(title="Set absolut starting velocity at which Fallschirm is opened [m/s]", value=x0[1], start=0,
                  end=1000, step=5)
...
def update_data(attrname, old, new):
    ...
    y1 = [entry[0] for entry in y]
    y2 = [entry[1] for entry in y]
    text_out.text = "The Maximum Force which acts on the Fallschirm is %d N, at time = %d s" % (
        max_force["max"][0], max_force["max"][1])
...
curdoc().add_root(
    column(row(fig, fig2), drag_slid, mass_slid, area_slid, time_slid, height_slid, open_slid, vel_slid, text_out,
           fig3))
