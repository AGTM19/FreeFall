...
def getInterpolatedValue(d, key):
    ...
    keys = __getKeyList__(d)

    valid_keys = __getValidKey__(keys, key)
    if valid_keys[0] == valid_keys[1]:
        return d[key]
    return __interpolate__(key, valid_keys, d)
...
def main():
    ...
    print("")
    print("x:", 5 * "\t", x)
    print("keys:", 4 * "\t", __getKeyList__(d))
    print("valid Keys:", 2 * "\t", __getValidKey__(__getKeyList__(d), x))
    print("interpolated value:\t", getInterpolatedValue(d, x))
